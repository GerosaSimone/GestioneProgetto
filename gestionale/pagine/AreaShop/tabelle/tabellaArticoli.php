...
<?php
$sql = "SELECT * FROM prodotto WHERE nascosto='0' ORDER BY id DESC";
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo "  <div class='col-md-3 mt-5 ' >
                        <div class='card shop rounded shadow-lg border-0 '>
                                <div class='card-body p-4 d-flex flex-column justify-content-between'>
                                    <div onclick='apriModal(this)' data-bs-whatever='" . $row['id'] . "'>
                                        <img src='img/uploadsProdotti/" . $row['foto'] . "' alt='' class='img-fluid d-block mx-auto mb-3 rounded fotoCarta' >
                                        <h4> <b>" . $row['nome'] . "</b>
                                        </h4>
                                        <p class=' text-muted '>" . $tipoTaglie . "</p>
                                        <p class=' text-primary font-italic pull-left '>Prezzo: " . $row['costoUnitario'] . " €</p>
                                    </div>
                                    <div>
